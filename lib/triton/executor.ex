defmodule Triton.Executor do
      ...
      |> Enum.reduce(Xandra.Batch.new(), fn {cql, prepared}, acc ->
        case prepared do
          ...
          prepared ->
            Xandra.Batch.add(
              acc,
              Xandra.prepare(Triton.Conn, cql, options),
              prepared
            )
        end
      end)

    with {:ok, %Xandra.Void{}} <- Xandra.execute(Triton.Conn, batch, options),
         do: {:ok, :success}
  end
  ...
  def execute(query, options \\ []) do
    with {:ok, query} <- Triton.Validator.coerce(query),
  ...
  defp execute_cql(conn, :stream, cql, nil, options) do
    with pages <- Xandra.stream_pages!(conn, cql, [], options) do
      results =
        pages
  ...
  defp execute_cql(conn, :select, cql, nil, options) do
    with {:ok, page} <- Xandra.execute(conn, cql, [], options),
  ...
  defp execute_cql(conn, _, cql, nil, options) do
    with {:ok, %Xandra.Void{}} <- Xandra.execute(conn, cql, [], options) do
      {:ok, :success}
    else
  ...
  defp execute_error({:ok, %Xandra.Page{} = page}) do
    case page |> Enum.to_list() |> List.first() do
      %{"[applied]" => applied} ->
        if applied,
          do: {:ok, :success},
          else: {:error, %{message: "Your operation was not applied."}}

      _ ->
        {:error, %{message: "Your operation was not applied."}}
    end
  end

  defp execute_error(error), do: error

  defp format_results(list) when is_list(list),
    do: list |> Enum.map(fn map -> string_to_atom_keys(map) end)

  defp format_results(_), do: nil

  defp string_to_atom_keys(list),
    do: list |> Enum.map(fn {k, v} -> {String.to_atom(k), v} end) |> Enum.into(%{})

  defp atom_to_string_keys(list),
    do: list |> Enum.map(fn {k, v} -> {to_string(k), v} end) |> Enum.into(%{})

  defp conn_for(query), do: query[:__metadata__].conn

  defp pool_for(query), do: query[:__metadata__].pool
end
