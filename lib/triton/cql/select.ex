defmodule Triton.CQL.Select do
  ...
  def build(query) do
    ...
    select(query[:select], query[:count], table, schema) <>
      where(query[:where]) <>
      order_by(query[:order_by] && List.first(query[:order_by])) <>
      limit(query[:limit]) <> allow_filtering(query[:allow_filtering])
  end
  ...
  defp select(fields, _, table, schema) when is_list(fields) do
    ...
    filtered_fields =
      MapSet.intersection(MapSet.new(req_fields), MapSet.new(schema_fields)) |> Enum.into([])
  ...
  defp where_fragment({k, :in, v}),
    do: "#{k} IN (#{v |> Enum.map(fn v -> value(v) end) |> Enum.join(", ")})"
